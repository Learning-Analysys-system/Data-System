...
# 3. HÀM CHÈN DỮ LIỆU
# =========================
def insert_dim_actor(cursor, actor):
    account_name = safe_get(actor, "account", "name")
    account_homepage = safe_get(actor, "account", "homePage")
...
def insert_dim_verb(cursor, verb):
    verb_id = verb.get("id")
    verb_display = safe_get(verb, "display", "en")
    if not verb_id:
        return
    cursor.execute("""
...
def insert_activity_detail(cursor, activity):
    activity_id = activity.get("id")
    if not activity_id:
        return

    name = safe_get(activity, "definition", "name", "en")
    type_ = safe_get(activity, "definition", "type")
...
def insert_dim_context(cursor, context):
    context_registration = context.get("registration")
    context_language = context.get("language")
    context_platform = context.get("platform")

    ext_info = safe_get(context, "extensions", "http://lrs.learninglocker.net/define/extensions/info", default={})
...
def insert_bridge_context_activity(cursor, context_id, context_activities):
    ...
    for role, acts in context_activities.items():
        if isinstance(acts, list):
            for act in acts:
                ...
                if not activity_id:
                    continue
                insert_activity_detail(cursor, act)
                cursor.execute("""
                    IF NOT EXISTS (
...
def insert_fact_statement(cursor, stmt, context_id):
    statement_id = stmt.get("id")
    actor_name = safe_get(stmt, "actor", "account", "name")
    verb_id = safe_get(stmt, "verb", "id")
    object_id = safe_get(stmt, "object", "id")
    timestamp = stmt.get("timestamp")
    stored = stmt.get("stored")
    version = stmt.get("version")
    authority_name = safe_get(stmt, "authority", "account", "name")
    authority_home = safe_get(stmt, "authority", "account", "homePage")
